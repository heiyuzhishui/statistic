...
# 标签映射字典
LABEL_MAPPING = {
    'shoes': ['shoes', 'socks', 'Slippers', 'Socks', 'socks'],  # 添加所有脚部穿着物
    'bin': ['trash_can', 'trash'],  # 添加trash作为垃圾桶的变体
    'pedestal': ['base', 'Landing'],  # 添加Landing作为基座类
    'wire': ['cable', 'Cable', 'charger', 'Electric', 'Cable', 'wire'],  # 保持电线类设备
    'socket': ['plug_in_board'],  # 保持原样
    'cat': ['pet_cat', 'pet_unknown', 'Cat', 'Pet', 'pet_hairbolls'],  # 添加宠物相关标签
    'dog': ['pet_unknown', 'pet_dog', 'Pet', 'pet_toilet', 'pet_house'],  # 添加宠物相关设施
    'desk_rect': ['table', 'Table', 'dining_table', 'Dining_table', 'bedside_table'],  # 添加所有桌子变体
    'desk_circle': [],  # 与desk_rect保持一致
    'weighing-scale': ['weight_scale', 'Weighing'],  # 添加Weighing变体
    'key': ['keys'],  # 保持原样
    'person': ['person'],  # 保持原样
    'chair': ['chair'],  # 保持原样
    'couch': ['sofa', 'Sofa'],  # 保持原样
    'bed': ['bed', 'Bed'],  # 添加大写变体
    'tvCabinet': ['tv_cabinet', 'TV', 'Television'],  # 添加电视相关标签
    'fridge': ['refrigerator', 'Refrigerator'],  # 添加大写变体
    'television': ['screen', 'TV', 'Television'],  # 添加电视相关标签
    'washingMachine': ['washing_machine', 'Washing'],  # 添加Washing变体
    'electricFan': ['Fan'],  # 保持原样
    'remoteControl': [],  # 保持原样
...
if __name__ == "__main__":
    json_directory = "/media/depth/v/20241224/20240928数据10544/2"  # Replace with your JSON files path
    xml_directory = "/media/depth/v/20241224_/20240928数据10544/2"  # Replace with your desired XML output path
